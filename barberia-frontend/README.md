...
- Copia `.env.example` a `.env`. Variables `PUBLIC_*` son visibles en el navegador.
- **Vercel**: en el proyecto, **Root Directory** = `barberia-frontend`. Añade `PUBLIC_API_URL` y `PUBLIC_SITE_URL` (URL de producción del frontend).
- **Backend (CORS)**: `FRONTEND_URL` en Nest debe coincidir con la URL del sitio en Vercel.
- URLs del API en TypeScript: `src/lib/config.ts` (`getApiV1Url()`).
